...
export async function action({ context, request }: ActionFunctionArgs) {
  ...
  // if (!consent) {
  //   return json({ subscriber: null, consentError: "You haven't agreed to the Privacy Policy" })
  // }
  ...
  const alreadySubscribed = existing?.customer?.emailMarketingConsent?.marketingState === 'SUBSCRIBED'
...
async function returnSuccess({
  ...
  session.set('emailMarketingConsent', subscriber.emailMarketingConsent.marketingState)
  return json(
 ...
 * @param context
 */
async function getCustomerConsent({ email, context }: { email: string; context: ActionFunctionArgs['context'] }) {
...
async function updateCustomerMarketingConsent({
  ...
  const consentUpdatedAt = new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60000).toISOString()
  ...
  const { customerEmailMarketingConsentUpdate } = await context.admin(UPDATE_CUSTOMER_MARKETING_CONSENT, {
    variables: { input }
  })
