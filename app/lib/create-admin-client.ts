...
export type AdminClient = (query: string | null, options: { variables: object | null }) => Promise<AdminResponse>
...
export function createAdminClient({
  ...
  const admin: AdminClient = async function (
    ...
    if (!request.ok) {
      throw new Error(`graphql api request not ok ${request.status} ${request.statusText}`)
    }

    const response: AdminAPIResponse = (await request.json()) as AdminAPIResponse
